...
const Administrator = () => {
  ...
  const toggleUserStatus = async (username, enable) => {
    try {
      ...
      showSuccessToast(`Usuario ${enable ? 'habilitado' : 'deshabilitado'} correctamente.`);
    } catch (error) {
  ...
  return (
    <div className="relative sm:w-[70vw] bg-black border-solid border-2 border-black rounded-3xl place-content-center">
      ...
      <section className="flex flex-col rounded-3xl w-[80vw] sm:w-full h-[90vh] sm:h-[90vh] sm:flex-row">
        <div
          className="flex flex-col md:flex-row w-full md:w-[55vw] h-[75vh] md:h-full pb-20 rounded-bl-3xl text-center text-lg md:text-2xl bg-white"
        >
          <div className="flex justify-start items-center md:bg-fondo rounded-bl-3xl md:h-[90vh] p-4 md:w-[24vw] lg:w-[20vw] md:space-y-20">
            <button className="rounded-3xl text-center text-xl lg:text-2xl text-white bg-black h-[7vh] w-[22vw] md:w-[15vw] lg:w-[16vw] transition duration-200"
              onClick={() => navigate("/admin-profile")}>
              Volver
            </button>
          </div>

          <h1 className="flex justify-evenly content-center w-full h-full text-2xl lg:text-3xl font-bold p-0 md:p-4">VISUALIZAR USUARIOS</h1>
        </div>
